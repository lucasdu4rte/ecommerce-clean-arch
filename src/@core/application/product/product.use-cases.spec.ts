...
const products = [buildProduct({ id: 1 }), buildProduct({ id: 2, name: "AirPods Pro", price: 249 })];
...
describe("product use cases", () => {
  ...
  it("propagates the gateway failure for an unknown product", async () => {
    ...
    await expect(new GetProductUseCase(gateway).execute(99)).rejects.toThrow("Product 99 not found");
  });
});
